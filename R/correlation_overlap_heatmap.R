...
#' @param cor.method A string specifying the correlation method
#' (pearson, kendall, spearman).
#' @param tissue.names A vector of strings with length of the number of columns
...
#'                             z = c(-1, 0, NA)), tissue.names = c("A", "B", "C"))
#'
#' @export
#' @import corrplot
#' @import psych
...
correlation_overlap_heatmap <- function(betas, cor.method = "pearson",
                                        tissue.names = 1:ncol(betas)){
  if (ncol(betas) == 0){
    return(NULL)
  }

  test_cor <- psych::corr.test(betas, method = cor.method, adjust = "fdr")
  corr_coef <- test_cor$r
  colnames(corr_coef) <- tissue.names
  row.names(corr_coef) <- tissue.names
  n_mat <- test_cor$n
  colnames(n_mat) <- tissue.names
  row.names(n_mat) <- tissue.names
  colnames(n_mat) <- tissue.names
  rownames(n_mat) <- tissue.names
